...
export default function KeysPage() {
  ...
  return <div className="grid gap-6 lg:grid-cols-[360px_1fr]">
    <Card>
      ...
      {createdKey && <div className="mt-4 rounded-lg border border-amber-500/30 bg-amber-500/10 p-3"><p className="mb-2 text-xs text-text-muted">Copy now. Key shown once.</p><code className="break-all text-sm">{createdKey}</code></div>}
    ...
    <Card>
      ...
      <div className="space-y-2">
        {keys.map((key) => <div key={key.id} className="flex items-center justify-between gap-3 rounded-lg border border-border p-3"><div><div className="flex items-center gap-2"><p className="font-medium">{key.name}</p><span className={`rounded-full px-2 py-0.5 text-[10px] font-medium ${key.isActive ? "bg-emerald-500/10 text-emerald-500" : "bg-red-500/10 text-red-500"}`}>{key.isActive ? "Active" : "Disabled"}</span></div><p className="text-xs text-text-muted">{key.keyPrefix && `${key.keyPrefix} · `}{key.ownerDisplayName && `${key.ownerDisplayName} · `}{key.allowedConnectionIds?.length ? `${key.allowedConnectionIds.length} selected connections` : "All accessible connections"}</p></div><div className="flex gap-2"><Button variant="secondary" onClick={() => setEditing({ ...key, allowedConnectionIds: [...(key.allowedConnectionIds || [])] })}>Edit</Button><Button variant="secondary" onClick={() => remove(key.id)}>Delete</Button></div></div>)}

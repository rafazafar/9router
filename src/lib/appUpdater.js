...
// Collect PIDs of all zrouter-related processes (excluding current)
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        // Match anything running from zrouter install dir or wrapper cli.js
        const isAppProcess = lower.includes("zrouter") ||
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        const isAppProcess = line.includes("zrouter") ||
...
function getDataDir() {
  ...
  if (process.platform === "win32") {
    return path.join(process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"), "zrouter");
  }
  return path.join(os.homedir(), ".zrouter");
}
...
// Resolve npx/zrouter binary to relaunch after update (cross-platform)
function resolveRelaunchCommand() {
  const isWin = process.platform === "win32";
  // Prefer `npx zrouter` — works regardless of global bin path changes after npm i -g

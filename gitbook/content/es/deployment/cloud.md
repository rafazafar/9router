...
Despliega zRouter en VPS o Docker para acceso remoto y uso en producción.
...
| `DATA_DIR` | `~/.zrouter` | Ruta de almacenamiento de la base de datos |
...
```bash
sudo mkdir -p /var/lib/zrouter
sudo chown $USER:$USER /var/lib/zrouter
```
...
Crea `/etc/nginx/sites-available/zrouter`:
...
sudo ln -s /etc/nginx/sites-available/zrouter /etc/nginx/sites-enabled/
...
# Si NO usas proxy reverso, permite los puertos de zRouter
...
tar -czf zrouter-backup-$(date +%Y%m%d).tar.gz /var/lib/zrouter

# Respaldo automatizado diario (agregar a crontab)
0 2 * * * tar -czf /backups/zrouter-$(date +\%Y\%m\%d).tar.gz /var/lib/zrouter
```
...
```bash
# Verificar si zRouter está corriendo
pm2 status
...
sudo chown -R $USER:$USER /var/lib/zrouter
chmod 755 /var/lib/zrouter
```

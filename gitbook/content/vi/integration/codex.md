...
Tích hợp zRouter với OpenAI Codex CLI để định tuyến request API OpenAI qua hệ thống routing thông minh của zRouter.
...
zRouter cung cấp các model Codex sau:
...
1. Xác minh API key đúng trong zRouter dashboard
...
1. Xác minh tên model khớp với cấu hình zRouter
2. Kiểm tra kết nối provider OpenAI đang hoạt động trong zRouter dashboard
...
Đảm bảo bạn đã cấu hình API key trong zRouter cloud dashboard.

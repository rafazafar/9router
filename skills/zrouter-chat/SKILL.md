...
- `POST $ZROUTER_URL/v1/chat/completions` — OpenAI format
- `POST $ZROUTER_URL/v1/messages` — Anthropic format

## Discover

```bash
curl $ZROUTER_URL/v1/models | jq '.data[].id'
# Per-model metadata (contextWindow, params)
curl "$ZROUTER_URL/v1/models/info?id=openai/gpt-4o"
```
...
```bash
curl -X POST $ZROUTER_URL/v1/chat/completions \
...
const client = new OpenAI({ baseURL: `${process.env.ZROUTER_URL}/v1`, apiKey: process.env.ZROUTER_KEY });
...
```bash
curl -X POST $ZROUTER_URL/v1/messages \
  -H "Authorization: Bearer $ZROUTER_KEY" \
